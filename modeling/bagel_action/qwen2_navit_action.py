...
class PackedAttentionMoT(Qwen2Attention):
    def __init__(self, config, layer_idx: Optional[int] = None):
        ...
        self.q_proj_moe_act = nn.Linear(self.action_hidden_size, self.num_heads * self.head_dim, bias=True)
        self.k_proj_moe_act = nn.Linear(self.action_hidden_size, self.num_key_value_heads * self.head_dim, bias=True)
        self.v_proj_moe_act = nn.Linear(self.action_hidden_size, self.num_key_value_heads * self.head_dim, bias=True)
        self.o_proj_moe_act = nn.Linear(self.num_heads * self.head_dim, self.action_hidden_size, bias=False)
    ...
    def forward_train(
        ...
        packed_sequence_act = packed_sequence[packed_act_token_indexes, :self.action_hidden_size]
        ...
        packed_key_states[packed_act_token_indexes] = self.k_proj_moe_act(packed_sequence_act)

        packed_value_states[packed_und_token_indexes] = self.v_proj(packed_sequence_und)
        packed_value_states[packed_gen_token_indexes] = self.v_proj_moe_gen(packed_sequence_gen)
        packed_value_states[packed_act_token_indexes] = self.v_proj_moe_act(packed_sequence_act)
        ...
        packed_attn_output_[packed_act_token_indexes,  :self.action_hidden_size] = \
            self.o_proj_moe_act(packed_attn_output[packed_act_token_indexes])
    ...
    def forward_inference(
        ...
        if mode == 'und':
            ...
            packed_text_query_sequence = packed_query_sequence[packed_text_indexes]
            packed_act_query_sequence = packed_query_sequence[packed_act_token_indexes, :self.action_hidden_size]
        ...
        if mode == 'und':
            ...
            packed_attn_output[packed_text_indexes] = self.o_proj(packed_attn_output[packed_text_indexes])
            packed_attn_output[packed_act_token_indexes, :self.action_hidden_size] = \
                self.o_proj_moe_act(packed_attn_output[packed_act_token_indexes])
...
class Qwen2MoTDecoderLayer(nn.Module):
    def __init__(
        ...
        self.action_hidden_size = config.action_hidden_size
        mlp_act_config = deepcopy(config)
        mlp_act_config.hidden_size = config.action_hidden_size
        mlp_act_config.intermediate_size = config.action_intermediate_size
        self.mlp_moe_act = Qwen2MLP(mlp_act_config)

        self.input_layernorm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.input_layernorm_moe_gen = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.input_layernorm_moe_act = Qwen2RMSNorm(config.action_hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm_moe_gen = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm_moe_act = Qwen2RMSNorm(config.action_hidden_size, eps=config.rms_norm_eps)
    ...
    def forward_train(
        ...
        packed_sequence_[packed_act_token_indexes, :self.action_hidden_size] = \
            self.input_layernorm_moe_act(packed_sequence[packed_act_token_indexes, :self.action_hidden_size])
        ...
        packed_sequence_[packed_act_token_indexes, :self.action_hidden_size] = self.mlp_moe_act(
            self.post_attention_layernorm_moe_act(packed_sequence[packed_act_token_indexes, :self.action_hidden_size])
        )
    ...
    def forward_inference(
        ...
        if not enable_taylorseer or (enable_taylorseer and self.current['type'] == 'full'):
            ...
            if mode == "und":
                ...
                packed_query_sequence_ = torch.zeros_like(packed_query_sequence)
                packed_query_sequence_[packed_text_indexes] = self.input_layernorm(packed_query_sequence[packed_text_indexes])
                packed_query_sequence_[packed_act_token_indexes, :self.action_hidden_size] = \
                    self.input_layernorm_moe_act(packed_query_sequence[packed_act_token_indexes, :self.action_hidden_size])
                packed_query_sequence = packed_query_sequence_
            ...
            if mode == "und":
                ...
                packed_act_query_sequence = packed_query_sequence[packed_act_token_indexes, :self.action_hidden_size]
                packed_text_query_sequence = self.post_attention_layernorm(packed_text_query_sequence).to(torch.bfloat16)
                packed_act_query_sequence = self.post_attention_layernorm_moe_act(packed_act_query_sequence).to(torch.bfloat16)

                packed_query_sequence_ = torch.zeros_like(packed_query_sequence).to(torch.bfloat16)
                packed_query_sequence_[packed_text_indexes] = self.mlp(packed_text_query_sequence)
                packed_query_sequence_[packed_act_token_indexes, :self.action_hidden_size] = self.mlp_moe_act(packed_act_query_sequence)
                packed_query_sequence = packed_query_sequence_
...
class Qwen2Model(Qwen2PreTrainedModel):
    def __init__(self, config):
        ...
        if self.use_moe:
            ...
            self.action_hidden_size = config.action_hidden_size
            self.norm_moe_act = Qwen2RMSNorm(config.action_hidden_size, eps=config.rms_norm_eps)
    ...
    def forward_train(
        ...
        if self.use_moe:
            ...
            packed_sequence_[packed_und_token_indexes] = self.norm(packed_sequence[packed_und_token_indexes]).to(torch.bfloat16)
            if self.config.freeze_und:
                packed_sequence_[packed_und_token_indexes] = packed_sequence_[packed_und_token_indexes].detach()
            packed_sequence_[packed_gen_token_indexes] = self.norm_moe_gen(packed_sequence[packed_gen_token_indexes]).to(torch.bfloat16)
            packed_sequence_[packed_act_token_indexes, :self.action_hidden_size] = \
                self.norm_moe_act(packed_sequence[packed_act_token_indexes, :self.action_hidden_size]).to(torch.bfloat16)

            return packed_sequence_
        else:
            return self.norm(packed_sequence).to(torch.bfloat16)
    ...
    def forward_inference(
        ...
        if self.use_moe:
            if mode == "und":
                packed_query_sequence = self.norm(packed_query_sequence).to(torch.bfloat16)
            elif mode == "gen":
                packed_query_sequence_ = torch.zeros_like(packed_query_sequence)
                packed_query_sequence_[packed_text_indexes] = self.norm(packed_query_sequence[packed_text_indexes]).to(torch.bfloat16)
                packed_query_sequence_[packed_vae_token_indexes] = self.norm_moe_gen(packed_query_sequence[packed_vae_token_indexes]).to(torch.bfloat16)
                packed_query_sequence = packed_query_sequence_
            elif mode == "act":
                packed_query_sequence_ = torch.zeros_like(packed_query_sequence)
                packed_query_sequence_[packed_text_indexes] = self.norm(packed_query_sequence[packed_text_indexes]).to(torch.bfloat16)
                packed_query_sequence_[packed_act_token_indexes, :self.action_hidden_size] = \
                    self.norm_moe_act(packed_query_sequence[packed_act_token_indexes, :self.action_hidden_size]).to(torch.bfloat16)
                packed_query_sequence = packed_query_sequence_
