...
from data.drive_dataset import DataConfig, DriveDataset, collate_wrapper
...
class ModelArguments:
    ...
    action_dim: int = field(
        default=3,
        metadata={"help": "Dimension of the action space."}
    )


@dataclass
class DataArguments:
    dataset_backend_args_path: str = field(
        default="./dataset_backend.yaml",
        metadata={"help": "Path to dataset backend args."}
    )
...
class TrainingArguments:
    ...
    save_optimizer_shards: bool = field(
        default=True,
        metadata={"help": "Save optimizer shards to checkpoint directories."}
    )
    ...
    resume_from: str = field(
        default=None,
        metadata={"help": "Explicit checkpoint path to resume from." }
    )
    ...
    step_size: float = field(
        default=10000,
        metadata={"help": "StepLR step size."}
    )
    gamma: float = field(
        default=0.5,
        metadata={"help": "StepLR gamma."}
    )
    min_lr: float = field(
        default=1e-7,
    ...
    mse_act_weight: float = field(
        default=1.0,
        metadata={"help": "Scaling factor for the action prediction MSE loss term."}
    )
...
def main():
    ...
    with open(data_args.dataset_backend_args_path, "r") as f:
        backend_args = yaml.load(f.read(), Loader=yaml.FullLoader)
    ...
    model, ema_model = FSDPCheckpoint.try_load_ckpt(
        training_args.resume_from, logger, model, ema_model, resume_from_ema=True
    )
    ...
    if training_args.lr_scheduler == 'cosine':
        ...
        scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer, training_args.step_size, training_args.gamma)
    else:
        raise ValueError

    # Setup packed dataloader
    backend_args["predict_image"] = training_args.predict_image
    logger.info(f'Data Backend arguments {backend_args}')
    dataset_config = DataConfig(backend_args=backend_args)
    # vit
    ...
    for curr_step, data in enumerate(train_loader):
        ...
        mse_act = loss_dict["mse_act"]
        total_mse_act_tokens = torch.tensor(len(data['action_mse_loss_indexes']), device=device)
        dist.all_reduce(total_mse_act_tokens, op=dist.ReduceOp.SUM)
        mse_act = mse_act.mean(dim=-1).sum() * dist.get_world_size() / total_mse_act_tokens
        loss_dict["mse_act"] = mse_act.detach()
        loss = loss + mse_act * training_args.mse_act_weight

        if training_args.predict_image:
            mse = loss_dict["mse"]
        ...
        if curr_step % training_args.log_every == 0:
            ...
            wandb_log['total_mse_act_tokens'] = total_mse_act_tokens.item()
        ...
        if curr_step > 0 and curr_step % training_args.save_every == 0:
            ...
            FSDPCheckpoint.fsdp_save_ckpt(
                ...
                data_status=None, 
                save_optimizer_shards=training_args.save_optimizer_shards, 
            )
